...
mod tests {
    ...
    mod admin_route_access {
        ...
        async fn should_return_403_for_non_admin_on_get_job() {
            ...
            // Try to GET /api/v1/private/cmd/{job_id} (admin-only)
            let fake_job_id = Uuid::new_v4().to_string();
            let resp = ctx
                .test_server
                .get(format!("/api/v1/private/cmd/{}", fake_job_id))
                .with_token(&token)
                .send()
                .await
                .unwrap();
        ...
        async fn should_return_403_for_non_admin_on_delete_job() {
            ...
            // Try to DELETE /api/v1/private/cmd/{job_id} (admin-only)
            let fake_job_id = Uuid::new_v4().to_string();
            let resp = ctx
                .test_server
                .delete(format!("/api/v1/private/cmd/{}", fake_job_id))
                .with_token(&token)
                .send()
                .await
                .unwrap();
        ...
        async fn should_allow_admin_on_admin_routes() {
            ...
            // GET /api/v1/private/cmd/{job_id} should NOT return 403 for admin
            let fake_job_id = Uuid::new_v4().to_string();
            let resp = ctx
                .test_server
                .get(format!("/api/v1/private/cmd/{}", fake_job_id))
            ...
            // DELETE /api/v1/private/cmd/{job_id} should NOT return 403 for admin
            let resp = ctx
                .test_server
                .delete(format!("/api/v1/private/cmd/{}", fake_job_id))
                .with_token(&admin_token)
    ...
    mod admin_user_route_access {
        ...
        async fn should_return_403_for_non_admin_on_get_users_with_search() {
            ...
            let resp = ctx
                .test_server
                .get("/api/v1/private/admin/users?q=test&page=0&limit=10")
                .with_token(&token)
                .send()
                .await
                .unwrap();
        ...
        async fn should_return_403_for_non_admin_on_get_user() {
            ...
            let resp = ctx
                .test_server
                .get("/api/v1/private/admin/users/55")
                .with_token(&token)
                .send()
                .await
                .unwrap();
        ...
        async fn should_allow_admin_on_admin_user_routes() {
            ...
            let resp = ctx
                .test_server
                .get("/api/v1/private/admin/users")
                .with_token(&admin_token)
                .send()
                .await
                .unwrap();
        ...
        async fn should_allow_admin_on_search_without_pagination_params() {
            ...
            let resp = ctx
                .test_server
                .get("/api/v1/private/admin/users?q=test")
                .with_token(&admin_token)
                .send()
                .await
                .unwrap();
