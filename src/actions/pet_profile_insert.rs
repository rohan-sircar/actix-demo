...
pub fn create_full_pet_profile(
    ...
    use crate::schema::{pet_activities, pet_adoption_details, pet_basic_info,
    ...
    conn.transaction::<_, DomainError, _>(|txn| {
        // Insert basic pet info
        let pet_basic_info: PetBasicInfo = diesel::insert_into(pet_basic_info::table)
            .values(&pet_data)
            .get_result(txn)
            .map_err(|err| {
        ...
        let personality_traits: PetPersonalityTraits = diesel::insert_into(pet_personality_traits::table)
        ...
        let location_owner: PetLocationOwner = diesel::insert_into(pet_location_owner::table)
        ...
        let adoption_details: PetAdoptionDetails = diesel::insert_into(pet_adoption_details::table)
...
pub fn create_pet_profile_from_insert_data(
    ...
    let personality_data = insert_data.to_new_pet_personality_traits(&pet_id);
...
pub fn create_pet_location_owner(
    ...
    diesel::insert_into(pet_location_owner::table)
        ...
        .map_err(|err| {
            DomainError::new_internal_error(format!(
                "Failed to create location/owner info: {err}"
            ))
        })
}
...
pub fn create_pet_adoption_details(
    ...
    diesel::insert_into(pet_adoption_details::table)
        ...
        .map_err(|err| {
            DomainError::new_internal_error(format!(
                "Failed to create adoption details: {err}"
            ))
        })
}

/// Insert pet profile images
pub fn create_pet_profile_images(
    images: Vec<NewPetProfileImage>,
    conn: &mut DbConnection,
    ...
    diesel::insert_into(pet_profile_images::table)
        ...
        .map_err(|err| {
            DomainError::new_internal_error(format!(
                "Failed to create pet profile images: {err}"
            ))
        })
}