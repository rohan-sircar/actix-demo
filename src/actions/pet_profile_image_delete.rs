...
/// Delete a specific pet profile image by its UUID
pub fn delete_pet_profile_image_by_uuid(
    ...
    let _ = tracing::info!(
        "Deleting pet profile image {image_uuid} for pet profile {pet_uuid}"
    );

    // Find and delete the image by UUID, returning the deleted record
    let deleted_image = conn.transaction::<_, DomainError, _>(|txn| {
        diesel::delete(pet_profile_images.filter(uuid.eq(image_uuid)))
