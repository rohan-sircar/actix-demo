...
impl RedisCredentialsRepo {
    ...
    pub async fn create_session(
        ...
        let mut pipe = redis::pipe();
        pipe.atomic().hset(key, token, session_info_str).set_ex(
            expiry_key,
            "1",
            ttl_seconds,
        );
    ...
    pub async fn update_session_last_used_ws(
        ...
        if let Some(session_info) = mb_session_info {
            self.update_session_last_used(session_info, user_id, token)
                .await?;
        }
    ...
    pub async fn update_session_last_used(
        ...
        self.update_session(user_id, token, &session_info).await?;
