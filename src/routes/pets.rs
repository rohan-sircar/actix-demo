...
let image_url_for_log = image_url.clone();
let pet_id_for_log = pet_id.clone();
...
let _ = tracing::info!(
        "Successfully added image {image_id} to pet profile {pet_id}",
        image_id = added_image.id,
        pet_id = pet_id_for_log
    );
...
let image_urls_for_log = image_urls.clone();
let pet_id_for_log = pet_id.clone();
...
let _ = tracing::info!(
        "Successfully added {count} images to pet profile {pet_id}",
...
let _ = tracing::info!(
    "Successfully set image {image_id} as primary for pet profile {pet_id}",
